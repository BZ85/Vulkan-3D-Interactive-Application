...
// clang-format off
// define the tracy GPU profiler marco (will be used for marking GPU profiling zones in CommandBuffer functions)
...
// align up (returns the smallest multiple of alignment that is bigger than value)
...
VKAPI_ATTR VkBool32 VKAPI_CALL vulkanDebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT msgSeverity,
	...
	// skips messages that are less severe than "INFO" (filtering)
	if (msgSeverity < VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) {
    return VK_FALSE;
  }

  // determine the severity type (error or warning)
  ...
  if (isError) {
    lvk::VulkanContext* ctx = static_cast<lvk::VulkanContext*>(userData);
	 ...
	 // special handling for shader error
...
VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats, lvk::ColorSpace colorSpace) {
  ...
  // surface format consists of format and color space
  // format like RGBA8, BGRA8
  // color space like sRGB, linear RGB


  // detect whether the GPU prefers BGR-formatted swapchain images over RGB ones
  // pick the one (RGB or BGP) that appear first in the list
  ...
  // if cannot find the format, then default to the first avaiable format
  return formats[0];
}
...
struct VulkanContextImpl final {
  ...
  // necessary member fields for Tracy GPU
#if defined(LVK_WITH_TRACY_GPU)
...
// mandatory for non-coherent memory (CPU to GPU)
// ensure that host writes to the buffer mapped memory become available to GPU
void lvk::VulkanBuffer::flushMappedMemory(const VulkanContext& ctx, VkDeviceSize offset, VkDeviceSize size) const {
  ...
  if (LVK_VULKAN_USE_VMA) { // if VMA library is enabled
...
// mandatory for non-coherent memory (GPU to CPU)
...
// get the buffer's mapped data from GPU
void lvk::VulkanBuffer::getBufferSubData(const VulkanContext& ctx, size_t offset, size_t size, void* data) {
  ...
  if (!isCoherentMemory_) { // invalidate for non-coherent memory
...
// upload the data to the buffer's mapped memory on GPU
void lvk::VulkanBuffer::bufferSubData(const VulkanContext& ctx, size_t offset, size_t size, const void* data) {
  ...
  } else { // set the content of buffer to be 0 if the data pointer is NULL
    memset((uint8_t*)mappedPtr_ + offset, 0, size);
  }

  if (!isCoherentMemory_) {  // flush for non-coherent Memory
...
// precaches image views (as framebuffer attachments, and have only 1 mip level and 1 layer) inside imageViewForFramebuffer_ array
...
lvk::VulkanSwapchain::VulkanSwapchain(VulkanContext& ctx, uint32_t width, uint32_t height) :
  ...
  // check that the selected formats support presentation operation on the graphics queue family used for creating swapchain
  ...
  // add one more image to minImage
  // make sure we are not waiting on GPU to complete any operations (triple buffering)
  ...
  // choose usage flags for swapchain images
...
lvk::TextureHandle lvk::VulkanSwapchain::getCurrentTexture() {
  ...
  if (getNextImage_) {
	 ...
	 // ensure that the current swapchain image is finshed rendering, then acquire new image
    const VkSemaphoreWaitInfo waitInfo = { // not submitInfo, is used for vkWaitSemaphores, GPU-CPU
    ...
    VkSemaphore acquireSemaphore = acquireSemaphore_[currentImageIndex_];
	 // signal the acquireSemaphore when the swapchain image is acquired (function is running asynchronously)
    ...
    getNextImage_ = false;
	 ...
	 // inject (pass) the acquireSemaphore to the command buffer's waitSemaphore_
	 // so that the command buffer will wait until a swapchain image is acquired, then executes
...
lvk::VulkanImmediateCommands::VulkanImmediateCommands(VkDevice device, uint32_t queueFamilyIndex, const char* debugName) :
  ...
  // first flag to specify the command buffers can be individually reset
  // second flag to indicate that command buffer from this pool will have a short lifespan (transient)
  // transient means that they will be reset or free within a relatively short timeframe
  ...
  // preallocate all the command buffers from command pool
  ...
  for (uint32_t i = 0; i != kMaxCommandBuffers; i++) {
    ...
    // create one semaphore and once fence for each command buffer to enable our synchronization system
...
lvk::VulkanImmediateCommands::~VulkanImmediateCommands() {
  ...
  // wait for all command buffers to be processed before destroy the command pool, fences, and semaphores
  waitAll();
...
// reset (clean up) old submitted command buffers by checking their fences
void lvk::VulkanImmediateCommands::purge() {
  ...
  for (uint32_t i = 0; i != numBuffers; i++) {
    ...
    CommandBufferWrapper& buf = buffers_[(i + lastSubmitHandle_.bufferIndex_ + 1) % numBuffers];
	 ...
	 // skip the command buffer that isn't been created or is being recorded
    if (buf.cmdBuf_ == VK_NULL_HANDLE || buf.isEncoding_) {
      continue;
    }

	 // return the fence status without waiting
    const VkResult result = vkWaitForFences(device_, 1, &buf.fence_, VK_TRUE, 0);

    if (result == VK_SUCCESS) { // if fence is signaled (command has been finished)
...
// return a reference to the next available command buffer
const lvk::VulkanImmediateCommands::CommandBufferWrapper& lvk::VulkanImmediateCommands::acquire() {
  LVK_PROFILER_FUNCTION();

  if (!numAvailableCommandBuffers_) {
    purge(); // recycle processed command buffers and reset them to their inital state
  }
  ...
  current->isEncoding_ = true; // prevent the reuse of a command buffer that has already been accquired but has not yet been submitted
...
void lvk::VulkanImmediateCommands::wait(const SubmitHandle handle) {
  ...
  if (!LVK_VERIFY(!buffers_[handle.bufferIndex_].isEncoding_)) {
    // we are waiting for a buffer which has not been submitted (is still recording) - this is probably a logic error somewhere in the calling code
    return;
  }

  // after all fast paths, we block CPU until GPU work (this submit handle) is done
  VK_ASSERT(vkWaitForFences(device_, 1, &buffers_[handle.bufferIndex_].fence_, VK_TRUE, UINT64_MAX));

  purge(); // we are sure at least one command buffer available to be reclaimed
}
...
void lvk::VulkanImmediateCommands::waitAll() {
  ...
  // waiting for all submitted command buffers to be completed
  ...
  purge(); // reclaim all completed command buffers
}

// determine whether a given command buffer (identified by a SubmitHandle) is finished executing on the GPU and can be safely reused
bool lvk::VulkanImmediateCommands::isReady(const SubmitHandle handle, bool fastCheckNoVulkan) const {
  ...
  // As the last step, we query Vulkan for the fence status
  return vkWaitForFences(device_, 1, &buf.fence_, VK_TRUE, 0) == VK_SUCCESS;
}

// we need to make sure that the command buffers in the same queue are processed sequentially one by one (use semaphore to ensure it)
lvk::SubmitHandle lvk::VulkanImmediateCommands::submit(const CommandBufferWrapper& wrapper) {
  ...
  // set wait semaphores (block current command buffer until another semaphore is signaled)
  VkSemaphoreSubmitInfo waitSemaphores[] = {{}, {}};
  uint32_t numWaitSemaphores = 0;
  if (waitSemaphore_.semaphore) { // injected by waitSemaphore() function, can be an acquire semaphore from a swapchain image(ensure the command buffer will wait until a swapchain image is acquired)
    waitSemaphores[numWaitSemaphores++] = waitSemaphore_;
  }
  if (lastSubmitSemaphore_.semaphore) { // wait this semaphore to be signaled (when the last command buffer is submitted and finished execution)
    waitSemaphores[numWaitSemaphores++] = lastSubmitSemaphore_;
  }

  // set signal semaphores (signal a semphore when the current command buffer has finished execution)
  // the first semaphore will be the lastSubmitSemaphore for next submitted command buffer
  // to chain command buffer together
  VkSemaphoreSubmitInfo signalSemaphores[] = {
      VkSemaphoreSubmitInfo{.sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO,
                            .semaphore = wrapper.semaphore_, // signal the current command buffer's semaphore when the buffer is submitted and finished execution
                            .stageMask = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT},
      {},
  };
  uint32_t numSignalSemaphores = 1;
  if (signalSemaphore_.semaphore) { // optional timeline semaphore
  ...
  const VkCommandBufferSubmitInfo bufferSI = {
      ...
      .commandBuffer = wrapper.cmdBuf_, // command buffer from current commandBufferWrapper object
  };
  const VkSubmitInfo2 si = {
      .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO_2,
      .waitSemaphoreInfoCount = numWaitSemaphores, // wait semaphore
      .pWaitSemaphoreInfos = waitSemaphores, // wait for this semaphore to be signaled before executing the current command buffer
      .commandBufferInfoCount = 1u,
      .pCommandBufferInfos = &bufferSI,
      .signalSemaphoreInfoCount = numSignalSemaphores, // signal semaphore 
      .pSignalSemaphoreInfos = signalSemaphores,  // signal this semaphore after the command buffer is submitted and finished executed
  };
  VK_ASSERT(vkQueueSubmit2(queue_, 1u, &si, wrapper.fence_));
  LVK_PROFILER_ZONE_END();

  // set the semaphore of current command buffer (just submitted) as the last submit semaphore (for next command buffer)
  lastSubmitSemaphore_.semaphore = wrapper.semaphore_;
  lastSubmitHandle_ = wrapper.handle_;
  // discard these two semaphores since they are meant to be used with exacly one command buffer
  waitSemaphore_.semaphore = VK_NULL_HANDLE;
  signalSemaphore_.semaphore = VK_NULL_HANDLE;

  // reset (set the isEncoding to be false, since the command buffer is submitted and recording is finished)
  const_cast<CommandBufferWrapper&>(wrapper).isEncoding_ = false;
  submitCounter_++; // used to set the submitId in the next SumbitHandle
...
// inject the signal into the command buffer submission
...
// the constructor for the VulkanPipeline builder (provide default values for numerous Vulkan data members that we don't want to set)
lvk::VulkanPipelineBuilder::VulkanPipelineBuilder() :
  ...
  rasterizationState_({
      ...
      .depthBiasEnable = VK_FALSE,  // deal with Z buffer fighting, like glPolygonOffset in OpenGL
...
VkResult lvk::VulkanPipelineBuilder::build(VkDevice device,
  ...
  // use the color blend states and attachments we prepared earlier in getVkpipeline function
  ...
  // necessary for Vulkan 1.3 dynamic rendering feature
  ...
  // put everything (prepared above) together into VkGraphicsPipelineCreateInfo and call vkCreateGraphicsPipelines() to create Vulkan graphics pipeline
  const VkGraphicsPipelineCreateInfo ci = {
      .sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO,
      .pNext = &renderingInfo, // chain the dynamic rendering info
      .flags = 0,
      ...
      .renderPass = VK_NULL_HANDLE, // must set the render pass to be NULL for enabling dynamic rendering
      .subpass = 0,
...
void lvk::CommandBuffer::cmdDispatchThreadGroups(const Dimensions& threadgroupCount, const Dependencies& deps) {
  LVK_PROFILER_FUNCTION();
  // mark the GPU profiling zone here
...
void lvk::CommandBuffer::cmdBeginRendering(const lvk::RenderPass& renderPass, const lvk::Framebuffer& fb, const Dependencies& deps) {
  ...
  // this VkRenderingAttachmentInfo struct is only used for dynamic rendering
  // multiple color attachments are allowed which means the fragment shader can have multiple outputs (more than one render target) in a single render pass
  ...
  // used for setting the depth buffer when using dynamic rendering
  ...
  // VkRenderingInfo is only used for dynamic rendering
  // All the information from Framebuffer and RenderPass structs is filled into VkRenderingInfo
  const VkRenderingInfo renderingInfo = {
      ...
      .pDepthAttachment = depthTex ? &depthAttachment : nullptr, // fill the depth attachment into the VkRenderingInfo
  ...
  // dynamic rendering command, take the command buffer and VkRenderingInfo as parameters
...
void lvk::CommandBuffer::cmdBindVertexBuffer(uint32_t index, BufferHandle buffer, uint64_t bufferOffset) {
  ...
  // checks to ensure the correct usage of the buffer
...
void lvk::CommandBuffer::cmdBindIndexBuffer(BufferHandle indexBuffer, IndexFormat indexFormat, uint64_t indexBufferOffset) {
  ...
  // conversion of some enum types from LVK to Vulkan
...
void lvk::VulkanStagingDevice::bufferSubData(VulkanBuffer& buffer, size_t dstOffset, size_t size, const void* data) {
  ...
  if (buffer.isMapped()) { // if the destination buffer is host-visible (whose memory should be mapped), then just call the bufferSubData function of it
  ...
  while (size) { // we deal with the data size chunk by chunk
    ...
    // copy data into staging buffer ( staging buffer is always host-visible, so call this function directly)
    ...
    const VkBufferCopy copy = {
	 ...
	 // acquire a command buffer and issue Vulkan commands to copy buffer data
    const lvk::VulkanImmediateCommands::CommandBufferWrapper& wrapper = ctx_.immediate_->acquire();
    vkCmdCopyBuffer(wrapper.cmdBuf_, stagingBuffer->vkBuffer_, buffer.vkBuffer_, 1, &copy); // params: command buffer, src buffer, dst buffer
    VkBufferMemoryBarrier barrier = {
        .sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
        .srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT, // src access mask is the kind of memory access should be completed before the barrier
        .dstAccessMask = 0, // dst access mask is the kind of memory access can only start after the barrier
        .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
        .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
	 ...
	 // deduce the dst stage mask and the dst access mask
    ...
    if (buffer.vkUsageFlags_ & VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR) {
	 ...
	 // srcStageMask is the latest pipeline stage that must complete before the barrier
	 // dstStageMask is the earliest pipeline stage that can start after the barrier
    vkCmdPipelineBarrier(
        wrapper.cmdBuf_, VK_PIPELINE_STAGE_TRANSFER_BIT, dstMask, VkDependencyFlags{}, 0, nullptr, 1, &barrier, 0, nullptr);
    desc.handle_ = ctx_.immediate_->submit(wrapper); // submit the command buffer

	 // push this memory region with its submit handle back to the regions deque
	 // with the submit handle, it will not be selected in getNextFreeOffset function
...
void lvk::VulkanStagingDevice::imageData2D(VulkanImage& image,
  ...
  if (desc.size_ < storageSize) {
    waitAndReset(); // wait until a region that is bigger than the storage size occurs
    desc = getNextFreeOffset(storageSize);
  }

  // unable to upload the images whose size is larger than the staging buffer size
  ...
  // copy the data into the staging buffer firstly
  ...
  // set the aspect of image (that we are looking to) to be the color (other options are depth, stencil and so on)
  ...
  for (uint32_t mipLevel = 0; mipLevel < numMipLevels; ++mipLevel) {
    for (uint32_t layer = 0; layer != numLayers; layer++) {
      ...
      // 1. Transition initial image layout into TRANSFER_DST_OPTIMAL (through the barrier)
      lvk::imageMemoryBarrier2(wrapper.cmdBuf_,
                               image.vkImage_, // the destination of transfer operation
                               StageAccess{.stage = VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT, .access = VK_ACCESS_2_NONE},
                               StageAccess{.stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_WRITE_BIT},
                               VK_IMAGE_LAYOUT_UNDEFINED,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
      ...
      for (uint32_t plane = 0; plane != numPlanes; plane++) {
			// VkExtent2D describes the size of image
        const VkExtent2D extent = lvk::getImagePlaneExtent(
            ...
            plane);
		  // VkRect2D consists of a VkExtent2D and a position (offset)
        ...
        const VkBufferImageCopy copy = {
            // the offset for this level is at the start of all mip-levels plus the size of all previous mip-levels being uploaded
			   // desc.offset_ is the start address (in the staging buffer) of all mip-levels, offset is the size of all previously uploaded mip-levels 
      ...
      // 3. Transition TRANSFER_DST_OPTIMAL into SHADER_READ_ONLY_OPTIMAL (through the barrier)
		// ensure the image is ready for use in the normal rendering pipeline
      lvk::imageMemoryBarrier2(
          wrapper.cmdBuf_,
          image.vkImage_,
          StageAccess{.stage = VK_PIPELINE_STAGE_2_TRANSFER_BIT, .access = VK_ACCESS_2_TRANSFER_WRITE_BIT},
		...
		// advance the buffer offset to the next mip-level and layer
      offset += lvk::getTextureBytesPerLayer(imageRegion.extent.width, imageRegion.extent.height, texFormat, currentMipLevel);
    }
  }
  // track the layout for the entire VkImage (not for its individual subresources, tradeoff)
...
// return the next available memory region suitable for the requested size in staging buffer
lvk::VulkanStagingDevice::MemoryRegionDesc lvk::VulkanStagingDevice::getNextFreeOffset(uint32_t size) {
  ...
  // this will be returned if no available region that is big enough
  ...
  for (auto it = regions_.begin(); it != regions_.end(); ++it) {
    if (ctx_.immediate_->isReady(it->handle_)) {
      ...
      if (it->size_ >= requestedAlignedSize) {
        // It is big enough!
			// split the region, and set the remaining unused region
        ...
        SCOPE_EXIT {
          regions_.erase(it); // erase it from the regions deque
          if (unusedSize > 0) {
            // set the remaining unused size as a new region and add it to the deque
      ...
      // cache the largest available region that isn't as big as the one we're looking for
		// which means, if there is no region with enough size, cache the largest available one
  ...
  // regions.end() means the end of the deque (after the last element)
  // bestNextIt == regions_.end() means that the iterator finds nothing
  ...
  // push the unused size (as the remaining region) into the region deque
  if (unusedSize) {
...
void lvk::VulkanStagingDevice::waitAndReset() {
  LVK_PROFILER_FUNCTION_COLOR(LVK_PROFILER_COLOR_WAIT);
  // wait for all the regions in the deque to be finished using (to be free)
  for (const MemoryRegionDesc& r : regions_) {
    ctx_.immediate_->wait(r.handle_);
  };

  regions_.clear();
  regions_.push_front({0, stagingBufferSize_, SubmitHandle()}); // add a region that spans the entire staging buffer
}
...
lvk::ICommandBuffer& lvk::VulkanContext::acquireCommandBuffer() {
  ...
  // store a new lvk::CommandBuffer object and return a referent to it 
...
lvk::SubmitHandle lvk::VulkanContext::submit(lvk::ICommandBuffer& commandBuffer, TextureHandle present) {
  ...
  // collect GPU sampling information using TracyVkCollect function before submitting the next command buffer
#if defined(LVK_WITH_TRACY_GPU)
  TracyVkCollect(pimpl_->tracyVkCtx_, vkCmdBuffer->wrapper_->cmdBuf_);
#endif // LVK_WITH_TRACY_GPU

  if (present) {
	  // take an optional argument of a swapchan texture to be presented
	  // do proper layout transitioning for Vulkan image
  ...
  if (shouldPresent) {
    // if we a presenting a swapchain image, signal our timeline semaphore
	 // the signal value should the be the frame index that the current swapchain image will have when it is acquired next time
	 // so we need to plus the swapchain image number
    ...
    swapchain_->timelineWaitValues_[swapchain_->currentImageIndex_] = signalValue;
	 // inject the timeline semaphore as the signalSemaphore_ into the command buffer submission
  ...
  // after submission, retrieve the last submit semaphore, and pass it into swapchain
  // vkQueuePresent will wait for the command buffer is finished executed (the image is fully rendered by GPU)
  // then present the image
  if (shouldPresent) {
	  // wait for the semaphore to be signaled, then present the image
    swapchain_->present(immediate_->acquireLastSubmitSemaphore());
  }

  // deferred task should only be run when an submitHandle (VkFence) is ready
...
lvk::Holder<lvk::BufferHandle> lvk::VulkanContext::createBuffer(const BufferDesc& requestedDesc, const char* debugName, Result* outResult) {
  ...
  // if the staging buffer is disabled, then override the storage mode with hostVisible one
  ...
  // use the flags to ensure that the data can be transferred to and from this buffer
  ...
  // for each requested flag, enable the corresponding Vulkan one
  ...
  if (desc.usage & BufferUsageBits_Uniform) {
    usageFlags |= VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR;  // buffer device address feature (access buffer by pointers from shaders), instead of using descriptor set
  }
  ...
  // retrieve the Vulkan memory properties using a helper function
  ...
  if (desc.data) { // if there is initial data to be uploaded into a buffer
...
lvk::Holder<lvk::TextureHandle> lvk::VulkanContext::createTexture(const TextureDesc& requestedDesc,
  ...
  if (debugName && *debugName) {
    desc.debugName = debugName; // override the debug name (useful when creating multiple textures using same textureDesc object)
  }

  // convert LVK image format to Vulkan image format
  ...
  // dynamically construct the correct vk usage flag for an image
  if (desc.usage & lvk::TextureUsageBits_Sampled) {
    usageFlags |= VK_IMAGE_USAGE_SAMPLED_BIT;
  }
  if (desc.usage & lvk::TextureUsageBits_Storage) { // can be read or written in the shader (storage image)
    LVK_ASSERT_MSG(desc.numSamples <= 1, "Storage images cannot be multisampled");
    usageFlags |= VK_IMAGE_USAGE_STORAGE_BIT;
  }
  if (desc.usage & lvk::TextureUsageBits_Attachment) { // is a render target (attachment) or not
    usageFlags |= lvk::isDepthOrStencilFormat(desc.format) ? VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT
                                                           : VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
    if (desc.storage == lvk::StorageType_Memoryless) { // for Vulkan transient attachments (depth images can be) and lazily allocated memory
      usageFlags |= VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT;
    }
  }

  // always allow the image can be read back from GPU to CPU except the transient attachment
  ...
  // generate debug names for Vulkan image and image view objects
  char debugNameImage[256] = {0};
  char debugNameImageView[256] = {0};

  if (hasDebugName) { // prefix the debugName string with "Image:" or "Image View:", and print it into the char array (buffer)
  ...
  // determine the VkImageCreateFlags and the types of Vulkan image and image view
  ...
  case TextureType_3D:
    ...
    break;
  case TextureType_Cube: // cube textures have their own image view type and vkCreateFlag
  ...
  // create a wrapper object VulkanImage, which encapsulates all necessary VkImage-related properties
  ...
  const bool isDisjoint = numPlanes > 1; // if it's multiplanar image, then the memory locations are disjoint

  if (isDisjoint) {
    // some constraints for multiplanar image formats (like YCbCr)
  ...
  // fill the VkImageCreateInfo struct
  ...
  if (LVK_VULKAN_USE_VMA && numPlanes == 1) { // if VMA is enabled
    VmaAllocationCreateInfo vmaAllocInfo = {
        .usage = memFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT ? VMA_MEMORY_USAGE_CPU_TO_GPU : VMA_MEMORY_USAGE_AUTO,
    };

	 // when using VMA to create vkImage, we only need VkImageCreateInfo and VmaAllocationCreateInfo
	 // and no worry about the vkDeviceMemory part
    ...
    // handle memory-mapped buffers
	 // have memory-mapped images the same way as we have memory-mapped buffers
    if (memFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
      vmaMapMemory((VmaAllocator)getVmaAllocator(), image.vmaAllocation_, &image.mappedPtr_);
    }
  } else { // if VMA is not enabled, using the raw Vulkan API directly
    // create image
    ...
    const VkImage img = image.vkImage_;
	 ...
	 // chain the planeMemoryRequirementsInfo in pNext of imageMemoryRequirementsInfo
	 // to get the memory requirements for a specific plane of a multi-planar image
    ...
    for (uint32_t p = 0; p != numPlanes; p++) {
		 // get memory requirements for each plane
  ...
  // start to create VkImageView object
  // determine which aspect will be included in the image view
...
lvk::Holder<lvk::TextureHandle> lvk::VulkanContext::createTextureView(lvk::TextureHandle texture,
  ...
  if (image.vkUsageFlags_ & VK_IMAGE_USAGE_STORAGE_BIT) {
    if (!desc.swizzle.identity()) {
      // use identity swizzle for storage images
		// since storage images do not support swizzling
      image.imageViewStorage_ = image.createImageView(vkDevice_,
                                                      ...
                                                      desc.layer,
                                                      desc.numLayers,
                                                      {}, // empty VkComponentMapping object
                                                      nullptr,
                                                      debugName);
      LVK_ASSERT(image.imageViewStorage_ != VK_NULL_HANDLE);
    }
  ...
  // signals to VulkanContext that a texture has been created
  // and the bindless descriptor set needs to be updated
  awaitingCreation_ = true;
...
VkPipeline lvk::VulkanContext::getVkPipeline(RenderPipelineHandle handle) {
	...
	// retrieve a renderPipelineState struct from a pool using a provide handle
  ...
  // check if the descriptor set layout for this VkPipeline object has changed
  if (rps->lastVkDescriptorSetLayout_ != vkDSL_) {

	 // we have to delete the old VkPipeline and VkPipelineLayout object and create new ones
	 // if the descriptor set layout has changed 
  ...
  // if the pipeline belonging to this rps is not created before, then create it now (since it's lazily created)
  if (rps->pipeline_ != VK_NULL_HANDLE) {
    return rps->pipeline_;
  }

  // build a new Vulkan pipeline based on the data stored in the rps
  ...
  for (uint32_t i = 0; i != numColorAttachments; i++) {
    ...
    LVK_ASSERT(attachment.format != Format_Invalid); // skip the non-active colorAttachment
    colorAttachmentFormats[i] = formatToVkFormat(attachment.format); // helper function to convery LVK enumerations to Vulkan
	 // set up blending states for color attachments
  ...
  // retrieve shaderModule states (contain VkShaderModule objects) from the pool using opaque handles
  ...
  // prepare the vertex input state (vertexBindingDescription and vertexAttributeDescription)
  ...
  // populate the VkSpecializationInfo to describe specialization constants for this graphics pipeline
  VkSpecializationMapEntry entries[SpecializationConstantDesc::LVK_SPECIALIZATION_CONSTANTS_MAX] = {};

  // this specialization info will be attached to shader stage later when setting the VkPipelineShaderStageCreateInfo
  const VkSpecializationInfo si = lvk::getPipelineShaderStageSpecializationInfo(desc.specInfo, entries);

  // create pipeline layout

  { // code for retrieve the precalculated push constant sizes from shader modules
	 // the push constant sizes were precalculated when the shader modules were created from SPIRV
	 // add the current shader stage bit to the stage flags
...
#undef UPDATE_PUSH_CONSTANT_SIZE
    ...
    // duplicate for MoltenVK (Vulkan-over-Metal)
    ...
    const VkPipelineLayoutCreateInfo ci = {
	 ...
	 // create VkPipelineLayout object
  ...
  // the creation of VkPipeline objects is encapsulated in VulkanPipelineBuilder
  lvk::VulkanPipelineBuilder()
      ...
      .shaderStage(meshModule
                       ...
                       : lvk::getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_VERTEX_BIT, vertModule->sm, desc.entryPointVert, &si)) // vertex shader module, and attach the si (specialization constant info)
      .shaderStage(lvk::getPipelineShaderStageCreateInfo(VK_SHADER_STAGE_FRAGMENT_BIT, fragModule->sm, desc.entryPointFrag, &si))  // fragment shader module, and attach the si (specialization constant info)
      ...
      .build(vkDevice_, pipelineCache_, layout, &pipeline, desc.debugName); // create a VkPipeline object and store it in our RenderPipelineState structure, together with pipeline layout
...
lvk::Holder<lvk::RenderPipelineHandle> lvk::VulkanContext::createRenderPipeline(const RenderPipelineDesc& desc, Result* outResult) {
  ...
  // store all the values in RenderPipelineState struct, for lazily creating actual VkPipeline objects in getVkPipeline() function
  ...
  if (desc.specInfo.data && desc.specInfo.dataSize) {
    // copy into a local storage owned by the pipeline, out of RenderPipelineDesc
	 // specConstantDataStorage_ is the local memory storage belonging to the renderPipelineState, for specialization constants
...
void lvk::VulkanContext::destroy(BufferHandle handle) {
  ...
  if (LVK_VULKAN_USE_VMA) { // if VMA is enabled
    if (buf->mappedPtr_) {
      vmaUnmapMemory((VmaAllocator)getVmaAllocator(), buf->vmaAllocation_);
    }

	 // the deferredTask member function postpones the execution of its lambda argument
	 // until all previously submitted command buffers have finished processing
	 // (ensure that the buffer is no longer used by the GPU)
  ...
  } else { // if using direct Vulkan code without VMA
    if (buf->mappedPtr_) {
...
// destroy the vulkanImage objects (including VkImage and VkImageView objects) that we have created
void lvk::VulkanContext::destroy(lvk::TextureHandle handle) {
  LVK_PROFILER_FUNCTION_COLOR(LVK_PROFILER_COLOR_DESTROY);

  SCOPE_EXIT { // the code inside will run when the function ends
    texturesPool_.destroy(handle);
    awaitingCreation_ = true; // make the validation layers happy
  };

  // get the VulkanImage object through the texture handle
  ...
  // unmap the memory and destroy the image, and finally free the memory
  // VMA path
  if (LVK_VULKAN_USE_VMA && tex->vkMemory_[1] == VK_NULL_HANDLE) {
    ...
    deferredTask(std::packaged_task<void()>([vma = getVmaAllocator(), image = tex->vkImage_, allocation = tex->vmaAllocation_]() {
		 // vmaDestroyImage function destroys the image and also frees the memory
		 vmaDestroyImage((VmaAllocator)vma, image, allocation); 
    }));
  } else { // raw Vulkan API code path
    if (tex->mappedPtr_) {
...
lvk::Result lvk::VulkanContext::upload(lvk::BufferHandle handle, const void* data, size_t size, size_t offset) {
  ...
  // convert a buffer handle to a VulkanBuffer object through the pool
  lvk::VulkanBuffer* buf = buffersPool_.get(handle);
  ...
  // VulkanStagingDevice class encapsulates all the functionality to manage Vulkan staging buffers
...
void lvk::VulkanContext::flushMappedMemory(BufferHandle handle, size_t offset, size_t size) const {
  const lvk::VulkanBuffer* buf = buffersPool_.get(handle); // fetch the buffer through the buffer handle firstly

  LVK_ASSERT(buf);

  // call the buffer's flushMappedMemory function
...
lvk::Holder<lvk::ShaderModuleHandle> lvk::VulkanContext::createShaderModule(const ShaderModuleDesc& desc, Result* outResult) {
  ...
  // handle the branching for textual GLSL and binary SPIR-V shaders
...
lvk::ShaderModuleState lvk::VulkanContext::createShaderModuleFromSPIRV(const void* spirv,
  ...
  // initialize SPIRV-Reflect on the SPIR-V shader blob (spirv), and parses it into the mdl object
  // mdl contains the reflection metadata
  ...
  for (uint32_t i = 0; i < mdl.push_constant_block_count; ++i) {
	  // each block here is one constant range
	  // retrieve the size of push constants from the constant range from mdl
...
lvk::ShaderModuleState lvk::VulkanContext::createShaderModuleFromGLSL(ShaderStage stage,
  ...
  // inject a bunch of textual source code into the provided GLSL code
  // reduce the code duplication in the shader
  // things like declaring GLSL extension and helper functions for bindless rendering are injected here
  ...
  // call compile shader to create a SPIR-V binary blob
  std::vector<uint8_t> spirv;
  const Result result = lvk::compileShader(vkStage, source, &spirv, &glslangResource);

  // then call the createShaderModuleFromSPIRV function which is above
...
void lvk::VulkanContext::createInstance() {
  ...
  {
	 // classical and  canonicalway of retrieving lists of properties in Vulkan
    ...
    vkEnumerateInstanceLayerProperties(&numLayerProperties, layerProperties.data());
	 ...
	 // loop for checking whether our desired validation layers in supported in the system
  ...
  // retrieve the list of all available Vulkan instance extensions for printing out
  ...
  // collect instance extensions from all validation layers
	...
	// need to add platform-specific extensions which takes windows handle
	// and attaches a rendering surface to it
...
// exclude Android system when add validation features
#if !defined(ANDROID)
...
// disable the specific validation checks that have bugs
...
#undef LAYER_SETTINGS_BOOL32
  const VkLayerSettingsCreateInfoEXT layerSettingsCreateInfo = {
      ...
      .pNext = config_.enableValidation ? &features : nullptr, // pointer to VkValidationFeaturesEXT
  ...
  // use Volk library to load Vulkan entry points
  ...
  if (hasDebugUtils) {
    const VkDebugUtilsMessengerCreateInfoEXT ci = {
        ...
        .pfnUserCallback = &vulkanDebugCallback, // callback function
        .pUserData = this,
    };
  ...
  // log (print out) all instance extensions supported in the current system
...
uint32_t lvk::VulkanContext::queryDevices(HWDeviceType deviceType, HWDeviceDesc* outDevices, uint32_t maxOutDevices) {
  ...
  // local lambda function that convert a Vulkan enum into a LightweightVK enum
...
lvk::Result lvk::VulkanContext::initContext(const HWDeviceDesc& desc) {
  ...
  // get the physical device that we selected earlier
  ...
  // add extensions to allDeviceExtensions
  getDeviceExtensionProps(vkPhysicalDevice_, allDeviceExtensions);
  if (config_.enableValidation) {
    for (const char* layer : kDefaultValidationLayers) {
		 // check the extension that the validation layer brings in
  ...
  // retrieve all Vulkan features and properties for this physical device
  vkGetPhysicalDeviceFeatures2(vkPhysicalDevice_, &vkFeatures10_); // vk 1.1-1.3 is chained with 1.0
  ...
  // print information related to physical device, and a list of all supported extensions, for debugging
  ...
  // deal with the corner case that graphics and compute queue are the same (especially on some mobile GPUs)
  const uint32_t numQueues = ciQueue[0].queueFamilyIndex == ciQueue[1].queueFamilyIndex ? 1 : 2;

  // a device must support a swapchain object (for presenting frames to the screen)
  ...
  // user can add extra custom extensions
  ...
  // request all necessary features that will be used
  // deviceFeatures10 uses pEnabledFeatures to pass (but not pNext, 11-13 use pNext)
  // due to Vulkan 1.0 design (historical reason)
  // all the features (need to use) must be enabled manually, and nothing is enabled by default 
  ...
  // check extensions (whether there is missing extensions)
  {
  ...
  // check features
  // use C-macros to check all requested Vulkan features against actual available features
  {
...
#undef CHECK_FEATURE_1_3
  ...
  const VkDeviceCreateInfo ci = {
      ...
      .pNext = createInfoNext, // pass Vulkan 1.1 1.2 1.3 features
      .queueCreateInfoCount = numQueues,
      .pQueueCreateInfos = ciQueue,
      .enabledExtensionCount = (uint32_t)deviceExtensionNames.size(),
      .ppEnabledExtensionNames = deviceExtensionNames.data(),
      .pEnabledFeatures = &deviceFeatures10, // pass Vulkan 1.0 feature
  };
  ...
  // if VK_EXT_calibrated_timestamps is available or not
  if (hasCalibratedTimestamps_) { 
    // query the number of calibrateable time domains and fetch the list
  ...
  // check whether there is a host visible time domain (called C++ lambda here)
  ...
  if (hasHostQuery) { // if host querying (host-visible time domain) is supported, use CPU + GPU calibrated timestamp (best and accurate way)
  ...
  } else { // manual calibration needs command buffer, command pool and timestamps
    ...
    if (hasCalibratedTimestamps_) { // Uses TracyVkContextCalibrated and vkGetCalibratedTimestampsEXT callback if VK_EXT_calibrated_timestamps is supported, but host query is not supported
...
lvk::Result lvk::VulkanContext::initSwapchain(uint32_t width, uint32_t height) {
  ...
  // init swapchain through the constructor
...
// when grow descriptor pool, we will destroy the old descriptor pool and descriptor set layout, and create new ones
lvk::Result lvk::VulkanContext::growDescriptorPool(uint32_t maxTextures, uint32_t maxSamplers, uint32_t maxAccelStructs) {
  ...
  // ensure the number of resources is withing the hardware specific limits
  ...
  // deallocates any existing descriptor set layout and descriptor pool
  ...
  // create descriptor set layout bindings for different kinds of Vulkan resources
  // normally each descriptor type (e.g., textures, samplers, buffers) has a binding belonging to its own
  // kBinding_NumBindings usually equals the number of descriptor types supported in the shader
  ...
  // the struct used for setting descriptor indexing
...

  {
    ...
    const VkDescriptorPoolCreateInfo ci = {
        ...
        .flags = VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT,// necessary to support the VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT in descriptor set layout
        .maxSets = 1,
...
#undef ENSURE_BUFFER_SIZE
  ...
  if (LVK_VULKAN_USE_VMA) { // VMA path
    VmaAllocationCreateInfo vmaAllocInfo = {};
    ...
    // handle memory-mapped buffers
    // host visible buffer are mapped to the pointer for the entire lifetime of the buffer
    // so that we can access the data using C++ pointers
    if (memFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
      vmaMapMemory((VmaAllocator)getVmaAllocator(), buf.vmaAllocation_, &buf.mappedPtr_);
    }
  } else { // direct Vulkan code path (not using VMA library for allocating memory)
    // create buffer
    VK_ASSERT(vkCreateBuffer(vkDevice_, &ci, nullptr, &buf.vkBuffer_));
    ...
    // back the buffer with some memory
    {
      VkMemoryRequirements requirements = {};
      vkGetBufferMemoryRequirements(vkDevice_, buf.vkBuffer_, &requirements);
		// check if the memory type supports coherent memory (if not, then the flush and invalidate function need to be used)
      if (requirements.memoryTypeBits & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) {
        buf.isCoherentMemory_ = true;
      }
    ...
    // handle memory-mapped buffers
	 // host visible buffer are mapped to pointer for the entire lifetime of the buffer
	 // so that we can access the data using C++ pointers
    if (memFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
  ...
  if (usageFlags & VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT) {
    const VkBufferDeviceAddressInfo ai = {
	 ...
	 // can be used in shaders to access the buffer
...
void lvk::VulkanContext::checkAndUpdateDescriptorSets() {
  ...
  // once a new texture is created, the awaitingCreation flag is set to be true 
  ...
  // grow the descriptor pool if needed
  ...
  // all the uniform and storage buffers are accessed by device address
  // so they are not appearing in descriptor set
  ...
  // iterate over the texture pool and populate the VkDescriptorImageInfo structures
  for (const auto& obj : texturesPool_.objects_) {
    const VulkanImage& img = obj.obj_;
  ...
  // 2. Samplers (also use VkDescriptorImageInfo struct)
  ...
  // 3. Acceleration structures (for ray tracing only)
  ...
  // create one VkWriteDescriptorSet structure for each of our three bindings
...
#endif // LVK_VULKAN_PRINT_COMMANDS
	 // wait until the last command buffer is finished executed
    immediate_->wait(immediate_->getLastSubmitHandle()); // ensure Vulkan is not using the descriptor set when updating it
...
void lvk::VulkanContext::querySurfaceCapabilities() {
  ...
  for (const VkFormat& depthFormat : depthFormats) {
    ...
    vkGetPhysicalDeviceFormatProperties(vkPhysicalDevice_, depthFormat, &formatProps);
	 ...
	 // check whether GPU support this format when using optimal tiling
  ...
  // retrieve and store all the surface capabilities and surface formats
